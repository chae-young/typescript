...
//타입제한을 걸수있다
interface obj<T extends keyof string> {
    add:(a:T,b:T) => T;
}
 */
...
class Hero implements Card {
    // 클래스도 꼭 규칙이 있어야할때
    public att: number; // number | nudefined (인터페이스에 비해 좁은집합)
    public hp: number;
    public cost: number;
    public mine: boolean;
...
class Sub implements Card {
    // 클래스도 꼭 규칙이 있어야할때
    protected att?: number; // number | nudefined (인터페이스에 비해 좁은집합)
    protected hp?: number;
    private cost?: number;
...
interface Player {
    ...
    //test?:Card => test?:Card | undefined와 같음
}
//타입스크립트는 DIV를 인식못해유
...
// 클래스가 있으면 그중에서 쫄병인지 아닌지 구별
const isSub = function (data: Card): data is Sub {
    //TRUE가되면 DATA는 SUB가 되는거 아니면..OO ->타입가드 라고함
    if (data.cost) {
        return true;
    }
    return false;
...
function initiate() {
    ...
    createDeck({ mine: false, count: 5 }); // 상대 덱 생성 (알수있도록 객체로!)
